...
export default function Header() {
  ...
  useEffect(() => {
    ...
    return () => {
      const lockedScrollY = Number.parseInt(document.body.style.top || "0", 10) * -1;
      document.documentElement.style.overflow = "";
      document.body.style.overflow = "";
      document.body.style.position = "";
      document.body.style.top = "";
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-3 sm:pt-5">
      <nav
        className={cn(
          "mx-auto w-[calc(100%-2rem)] max-w-none rounded-[1.5rem] border transition-[background-color,border-color,box-shadow,backdrop-filter] duration-300 ease-out",
          isScrolled
